<template>
  <div>
    <div>
      <!-- <h1>合同会社わしら<small>Wasira LLC</small></h1> -->
      <img
        src="/img/logo/wasilogo01.png"
        width="400"
        alt="合同会社わしら|Wasira LLC"
      />
    </div>

    <div>
      <h2>合同会社わしらとは <small>[About]</small></h2>
      <h3>『自分』と『自分の周り』をちょっと幸せにし続けます</h3>
      <p>
        多様な価値観の渦巻く時代になり、多くの人の同様の価値観というのは薄れてきました。<br />
        これをすればいい、こう進めばいいというような『正解』の無い、人によっては迷いの多い時代になったのかも知れません。<br />
        しかし、これは自分を表現し自分であることで多数決ではない自分だけの幸せを探求できる時代であると考えています。<br />
        人生において働く時間という膨大な時間をちょっと幸せに、自分のために活用し、そこから生まれた価値で周りをちょっと幸せにする。<br />
        それの繰り返し、探求が自分を幸せにし、周りを幸せにしつつづけることで、自分や社会のあり方を変え、変化を作り、変化に乗りこの不確実な時代を一緒に乗りこなしていきましょう。<br />
        合同会社わしらは、WEB技術をコアとし、これらに向き合っていきます。<br />
      </p>
    </div>

    <div>
      <h2>大事にする価値観 <small>[Values]</small></h2>
      <ul>
        <li>HRT(謙虚・尊重・信頼)な心でお互い仕事をする</li>
        <li>素直な心で仕事をする</li>
        <li>透明性の高い仕事をする</li>
        <li>ムリ・ムダ・ムラの無い仕事をする</li>
      </ul>
    </div>

    <div>
      <h2>事業内容 <small>[Businesses]</small></h2>
      <ul>
        <li>猫を幸せにする事業</li>
        <li>
          IT部門委託事業
          <ul>
            <li>
              <a href="https://techmesse.com/tech/1278" target="_blank"
                >情報システム部門チームのアウトソーシング | TechMesse</a
              >
            </li>
          </ul>
        </li>
        <li>IT部門内製化支援事業</li>
      </ul>
    </div>

    <div>
      <h2>製品/サービス <small>[Services]</small></h2>
      <ul>
        <li>moreneko.club ※開発中</li>
      </ul>
    </div>

    <div>
      <h2>メンバー <small>[Members]</small></h2>
      <div>
        <h3>藤本 永一 <small>(Eiichi Fujimoto)</small></h3>
        <p>代表社員 / プロダクトエンジニア</p>
        <img src="/img/members/001-001.png" width="150" />
        <p>
          FAエンジニアからプログラマを目指して、専門学校で学び直し、デジタルマーケティング会社にWEBエンジニアとして再就職、社内システム、受託でのサイトやシステム作成、自社のSaaS開発を経てエンジニアチームのマネジメントも担当。経験を活かし、世界を変えたい人や、楽しく働ける環境を作りたい人へエンジニアリングでお手伝いするため独立。
        </p>
        <ul>
          <li>
            <a href="https://github.com/eiichi-worker" target="_blank"
              >GitHub</a
            >
          </li>
          <li>
            <a href="https://twitter.com/EiichiCat" target="_blank">Twitter</a>
          </li>
        </ul>
      </div>

      <h2>パートナー <small>[Partners]</small></h2>
      <div>
        <ul>
          <li>WEBデザイナー</li>
          <li>WEBエンジニア</li>
          <li>メンター/コーチング</li>
        </ul>
      </div>
    </div>

    <div>
      <h2>SNS <small>[SNS]</small></h2>
      <ul>
        <li>
          <a href="https://twitter.com/washira_co" target="_blank">Twitter</a>
        </li>
        <li>
          <a href="https://www.linkedin.com/company/washira" target="_blank"
            >linkedin</a
          >
        </li>
        <li>
          <a href="https://techmesse.com/corporation/865/tech" target="_blank"
            >TechMesse</a
          >
        </li>
        <li>
          <a href="https://github.com/washira-co" target="_blank">GitHub</a>
        </li>
      </ul>
    </div>

    <div>
      <h2>法人情報 <small>[Company]</small></h2>
      <table>
        <tr>
          <th>会社名</th>
          <td>合同会社わしら</td>
        </tr>
        <tr>
          <th>会社名(英語)</th>
          <td>Washira LLC</td>
        </tr>
        <tr>
          <th>所在地</th>
          <td>
            530-0001<br />
            大阪府大阪市北区梅田1丁目1-3<br />
            大阪駅前第3ビル 29階1-1-1号室<br />
            ※ 郵便物の送付のみ対応しております
          </td>
        </tr>
        <tr>
          <th>設立</th>
          <td>2022年03月09日</td>
        </tr>
        <tr>
          <th>代表社員</th>
          <td>藤本 永一</td>
        </tr>
        <tr>
          <th>問い合わせ</th>
          <td>
            リンク先のフォームよりご連絡ください<br />
            <a
              href="https://tayori.com/form/c787ef402de80671fcd7ddb66d6e27d987bb8210/"
              target="_blank"
              >合同会社わしら お問い合わせフォーム</a
            ><br />
            ※ メール/電話でのお問い合わせは原則受け付けておりません。
          </td>
        </tr>
        <tr>
          <th>その他</th>
          <td>
            <a
              href="https://www.houjin-bangou.nta.go.jp/henkorireki-johoto.html?selHouzinNo=2120003021277"
              target="_blank"
              >国税庁法人番号公表サイト</a
            >
          </td>
        </tr>
      </table>
    </div>
  </div>

  <div>
    <h2>方針・規約 <small>[Policies]</small></h2>
    <ul>
      <li>
        <a href="/policies/privacy-policy/">プライバシーポリシー</a>
      </li>
    </ul>
  </div>
</template>

<script setup>
const ctx = useRuntimeConfig();

definePageMeta({
  title: "トップページ",
});

// const { data } = await useFetch("/blog", {
//   baseURL: ctx.baseURL,
//   headers: {
//     "X-MICROCMS-API-KEY": ctx.apiKey,
//   },
// });
</script>
